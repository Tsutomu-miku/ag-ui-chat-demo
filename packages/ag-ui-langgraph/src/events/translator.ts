...
type DispatchEvent = (event: BaseEvent) => BaseEvent | null;
...
function isToolMessageLike(value: unknown): value is ToolMessageLike {
...
function isCommandLike(value: unknown): value is { update?: unknown } {
  if (!isRecord(value)) return false;
...
function commandToolMessages(output: { update?: unknown }): ToolMessageLike[] {
...
function getEventDataRecord(event: LangGraphStreamEvent): Record<string, unknown> {
