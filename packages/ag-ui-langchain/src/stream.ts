/**
 * AG-UI streaming utilities for LangChain AIMessageChunk streams.
 *
 * Converts LangChain streaming output into AG-UI protocol events.
 * Extracted from the demo and made generic for reuse.
 */
...
type ToolCallChunkValue = NonNullable<
...
/**
 * Merge stream event metadata (stepName, parentStepName) into an AG-UI event.
 * Skips undefined values so the event remains clean.
 */
...
function appendChunk(
  ...
  state.finalChunk = state.finalChunk
    ? state.finalChunk.concat(chunk)
    : chunk;
...
function closeTextIfNeeded(state: AIMessageStreamState): BaseEvent[] {
  if (!state.started || state.textClosed || !state.messageId) return [];
...
function emitToolCallEndEvents(
  state: AIMessageStreamState,
): BaseEvent[] {
  if (!state.finalChunk) return [];

  const events: BaseEvent[] = [];
  for (const toolCall of state.finalChunk.tool_calls || []) {
    const toolCallId = toolCall.id || uuid();
    const toolCallState = Array.from(
      state.toolCallStates.values(),
    ).find((item) => item.emittedId === toolCallId);
...
function emitFinalTextEndEvent(
  state: AIMessageStreamState,
): BaseEvent[] {
  if (!state.started || state.textClosed || !state.messageId) return [];
...
/**
 * Convert a LangChain AIMessageChunk stream into AG-UI protocol events.
 *
 * Returns the accumulated final AIMessageChunk via the generator return value.
 *
 * @param stream - AsyncIterable of LangChain BaseMessage chunks
 * @param metadata - Optional step metadata to attach to all events
 */
export async function* eventsFromAIMessageStream(
  ...
  for await (const chunk of stream) {
    ...
    for (const toolCallChunk of chunk.tool_call_chunks || []) {
      for (const event of emitToolCallChunkEvents(
        state,
        toolCallChunk,
      )) {
        yield withStreamEventMetadata(event, metadata);
