...
import { useAgentChat, FRONTEND_TOOLS } from "../hooks/useAgentChat";
...
export function ChatPanel({
  ...
  }, [thread?.messages, streamingContent, streamingToolCalls, pendingToolCalls]);
    ...
    <main className="chat-panel">
      ...
      <div className="messages-container">
        {!hasMessages && !isStreaming && (
          <div className="welcome-screen">
            ...
            <p>Best-practice demonstration of the AG-UI protocol with LangChain</p>
            <div className="feature-grid">
              <div className="feature-card" onClick={() => setInput("What's the weather like in Tokyo?")}>
                <span className="feature-emoji">🌤</span>
                <span>Weather lookup<br /><small>Backend tool</small></span>
              </div>
              <div className="feature-card" onClick={() => setInput("Search for the latest news about AI agents")}>
                <span className="feature-emoji">🔍</span>
                <span>Web search<br /><small>Backend tool</small></span>
              </div>
              <div className="feature-card" onClick={() => setInput("Calculate (23 * 45) + (67 / 3)")}>
                <span className="feature-emoji">🧮</span>
                <span>Math calculator<br /><small>Backend tool</small></span>
              </div>
              <div className="feature-card" onClick={() => setInput("I need to deploy the production server, please confirm this action")}>
                <span className="feature-emoji">✅</span>
                <span>Confirm action<br /><small>Frontend tool</small></span>
              </div>
            </div>
            <p className="welcome-sub">
              <strong>Backend tools</strong> execute on the server. <strong>Frontend tools</strong> require your interaction.
            </p>
          </div>
        )}
      ...
      <div className="input-container">
        <div className="input-wrapper">
          ...
          />
          {isStreaming ? (
            <button className="send-btn stop" onClick={stopStreaming} title="Stop generating">
        ...
        <p className="input-hint">Enter to send · Shift+Enter for new line · Frontend tools require your confirmation</p>
      </div>
    </main>
  );
}
